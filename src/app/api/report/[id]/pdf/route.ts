...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch white-label settings for AGENCY users
  const user = await prisma.user.findUnique({ where: { id: userId }, select: { plan: true, whitelabelLogo: true, whitelabelColor: true } })
  const brandName = user?.whitelabelLogo ? '' : 'PageScore Report'
  const brandLogo = user?.whitelabelLogo || ''
  const brandColor = user?.whitelabelColor || '#0ea5e9'
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .logo { font-size: 20px; font-weight: 700; color: ${brandColor}; }
...
<body>
  <div class="header">
    ${brandLogo ? `<img src="${brandLogo}" alt="Logo" style="height:32px">` : `<div class="logo">${brandName}</div>`}
  ...
  <div class="footer">${brandLogo ? 'Powered by AI Analysis' : 'Generated by PageScore &mdash; pagescore.app'}</div>
</body>
</html>`
